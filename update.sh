...
curl --request GET \
  --url "http://mapserver.bdl.lasy.gov.pl/ArcGIS/rest/services/Mapa_turystyczna/MapServer/77/query?where=objectId%3E0&f=geojson&resultOffset=2000&outFields=nadl,kuchenka,rdlp,les" \
  -o bushcraft_2000.json \
  --header 'accept: text/html' \
  --header 'accept-language: pl-pl' \
  --header 'connection: keep-alive' \
  --header 'host: mapserver.bdl.lasy.gov.pl' \
...
jq -cs '{type: .[0].type, crs: .[0].crs, features: (.[0].features + .[1].features)}' bushcraft_0.json bushcraft_1000.json bushcraft_2000.json > bushcraft.json
